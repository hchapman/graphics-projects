...
{
    // We are starting to render this frame, so update the FPS timer
    if (!frames_rendered) {
        start_time = clock();
    } else if (!(frames_rendered % FRAMECOUNT)) {
        float ttime = (float)(clock() - start_time) / CLOCKS_PER_SEC;
        std::cout << FRAMECOUNT << " frames rendered. Total time (s): " << ttime <<
            "  Average FPS (fr/s): " << FRAMECOUNT / ttime << std::endl;
        start_time = clock();
        frames_rendered = 0;
    }
...
void SWRenderContext::rasterize(float p[3][4], 
    ...
    if (lineAB ^ lineAC) {
        ...
        if (type == TWO_LEVEL) {
            ...
            int xsub_amt, ysub_amt;
            xsub_amt = (bBox[2] - bBox[0]) / SUBDIV;
            ysub_amt = (bBox[3] - bBox[1]) / SUBDIV;

            for (int xsub = 0; xsub < SUBDIV; xsub++) {
                for (int ysub = 0; ysub < SUBDIV; ysub++) {
                    // Check the four corners of the box. If they are all outside 
                    // the range of any given barycentric coord, ignore the box.

                    Vector3<float> divPixel[4];
                    // Determine our subbox corner coordinates
                    divPixel[0] = Vector3<float> (bBox[0] + xsub_amt*xsub, 
                                                  bBox[1] + ysub_amt*ysub, 
                                                  1);
                    divPixel[1] = Vector3<float> ((xsub == SUBDIV - 1 ? 
                                                   bBox[2] : 
                                                   bBox[0] + xsub_amt*(xsub+1)),
                                                  bBox[1] + ysub_amt*ysub, 
                                                  1);
                    divPixel[2] = Vector3<float> ((xsub == SUBDIV - 1 ? 
                                                   bBox[2] : 
                                                   bBox[0] + xsub_amt*(xsub+1)),
                                                  (ysub == SUBDIV - 1 ? 
                                                   bBox[3] :
                                                   bBox[1] + ysub_amt*(ysub+1)),
                                                   1);
                    divPixel[3] = Vector3<float> (bBox[0] + xsub_amt*xsub,
                                                  (ysub == SUBDIV - 1 ? 
                                                   bBox[3] :
                                                   bBox[1] + ysub_amt*(ysub+1)),
                                                   1);

                    float abc[4][3]; // Barycentric coordinates of the corners
                
                    // Loop through and calculate the barycentric coordinates
                    for (int i = 0; i < 4; i++) {
                        abc[i][1] = (normAC^(divPixel[i] - pixels[0])) / 
                            (normAC^lineAB);
                        abc[i][2] = (normAB^(divPixel[i] - pixels[0])) / 
                            (normAB^lineAC);

                        abc[i][0] = (1 - abc[i][1] - abc[i][2]);
                    }

                    // While our box has not been proven to not intersect the 
                    // triangle, keep checking the corners and coordinates

                    bool possible = true;
                    for (int k = 0; possible && k < 6; k++) {
                        bool outside = true;
                        for (int i = 0; i < 4; i++) {
                            outside = outside && (k / 3 == 0 ? 
                                                  (abc[i][k%3] < 0) : 
                                                  (abc[i][k%3] > 1));
                        }
                        // If outside is true, then all four coordinates
                        // had the same b.c. either <0 or >1, and so don't intersect
                        possible = possible && !outside;
                    }

                    // If our box hasn't been shown to not intersect
                    if (possible) {
                        for (int x = divPixel[0][0]; 
                             x <= divPixel[1][0]; x++) {
                            for (int y = divPixel[0][1];
                                 y <= divPixel[3][1]; y++) {
